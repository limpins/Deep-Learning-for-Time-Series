...
from core import (MakeSeqData, Timer, get_Granger_Causality, get_mat_data, make_loader, set_device, train_test_split)
    ...
    for k in range(num_channel):
        ...
        for i in range(num_channel):
            ...
            for x_idx in channel_set:
                ...
                err_tmp = train_valid(
                    len(tmp_set), num_hidden, 1, f'checkpoints/with_NUE/{signal_type}_model_weights_{x_idx}.pth', x, y, train_loader, test_loader)
    ...
    for j in range(num_channel):
        for i in range(num_channel):
            if i not in im_IS[j]:
                ...
                err_cond[i, :, j] = train_valid(
                    len(im_IS[j]), num_hidden, 1, f'checkpoints/with_NUE/{signal_type}_model_weights_cond{j}.pth', x, y, train_loader, test_loader).squeeze()
